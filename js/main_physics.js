...
import Physijs from 'libs/threejs/plugins/physi'
...
export default class MainPhysics {
  constructor() {
    // 场景  
    this.scene = new Physijs.Scene({ worker: wx.createWorker('workers/request/physijs_worker.js') });
  ...
  start() {
    ...
    // 地面
    let ground_material = Physijs.createMaterial(new THREE.MeshBasicMaterial({ map: metal_texture }), .8, .3)
    this.ground = new Physijs.BoxMesh(new THREE.BoxGeometry(30, 1, 30), ground_material, 0)
    this.ground.receiveShadow = true
    this.scene.add(this.ground)

    // 盒子
    let box_meterial = Physijs.createMaterial(new THREE.MeshLambertMaterial({ map: wood_texture }), .6, .3)
    this.box = new Physijs.BoxMesh(new THREE.BoxGeometry(4, 4, 4), box_meterial)
    ...
    new THREE.JSONLoader().load(`${bastUrl}/sphere.json`,
      (geometry, materials) => {
        geometry.center()
        this.model_meterial = Physijs.createMaterial(new THREE.MeshLambertMaterial({ map: cloth_texture }), .6, .3)

        this.model = new Physijs.ConvexMesh(geometry, this.model_meterial)
